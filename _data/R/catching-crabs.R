...
dataset_url <- "https://opentraits.org/datasets/catching-crabs"
dataset <- "catching-crabs"
...
# Download files (using manual, was getting errors with automated download)
  ...
  download.file(url = "https://figshare.com/ndownloader/files/3158423",
                destfile = file.path("_data/R/temp/catching-crabs.csv"))
  
  traits <- read.csv(file.path("_data/R/temp/catching-crabs.csv"))
...
traits %>%
  ...
  pivot_longer(cols = 3:6, names_to = "traitNameVerbatim") %>%
  filter(!is.na(value),
         value != "") %>%
  mutate(scientificNameVerbatim = "Carcinus maenas") %>%
